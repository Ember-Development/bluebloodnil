...
export function Layout() {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!target.closest('.layout-notifications-dropdown') && !target.closest('.layout-notification-btn')) {
        setShowNotifications(false)
      }
      if (!target.closest('.layout-settings-dropdown') && !target.closest('.layout-settings-btn')) {
        ...
        <div className="layout-sidebar-top">
          ...
          <div className="layout-user-actions">
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="layout-notification-badge">{unreadCount > 99 ? '99+' : unreadCount}</span>
              )}
            </button>
            <button
      ...
      {showNotifications && (
        <div className="layout-notifications-dropdown">
          <div className="layout-notifications-header">
            <h3>Notifications</h3>
            {unreadCount > 0 && (
              <button className="layout-mark-all-read" onClick={handleMarkAllRead}>
                Mark all as read
              </button>
            )}
          </div>
          <div className="layout-notifications-list">
            {notifications.length === 0 ? (
              <div className="layout-notifications-empty">
                No notifications
              </div>
            ) : (
              notifications.map((notification) => (
                ...
                >
                  <div className="layout-notification-content">
                    <div className="layout-notification-title">{notification.title}</div>
                    <div className="layout-notification-message">{notification.message}</div>
                    <div className="layout-notification-time">
                      {new Date(notification.createdAt).toLocaleDateString()} {new Date(notification.createdAt).toLocaleTimeString()}
                    </div>
                  </div>
                  {!notification.read && <div className="layout-notification-dot" />}
                </div>
              ))
            )}
          </div>
      ...
      {showSettings && (
        <div className="layout-settings-dropdown">
          <div className="layout-settings-item">
            <span>Settings</span>
            <span style={{ fontSize: '0.8rem', color: 'var(--color-muted)' }}>Coming soon</span>
          </div>
          <div className="layout-settings-item">
            <span>Account</span>
            <span style={{ fontSize: '0.8rem', color: 'var(--color-muted)' }}>Coming soon</span>
