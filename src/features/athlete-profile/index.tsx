  ...
  return (
    <div className="profile-header">
      ...
      <div className="profile-stats">
        ...
        {typeof profile.nilScore === 'number' && (
          <div className="stat-item">
            <span className="stat-label">NIL Score</span>
            <span className="stat-value">{profile.nilScore}</span>
          </div>
        )}
      </div>
    </div>
      ...
      {badges.map((badge) => (
        <div key={badge.id} className="credibility-pill">
          ...
          <div className="credibility-text">
            ...
            {badge.description && <span className="credibility-description">{badge.description}</span>}
          </div>
        </div>
      ))}
    </div>
...
const MetricBand = ({ metrics }: { metrics: NILMetricTile[] }) => {
  ...
  return (
    <div className="metrics-band">
      {metrics.map((metric) => (
        <div key={metric.id} className={`metric-tile metric-${metric.emphasis ?? 'reach'}`}>
...
const ProfileTabs = ({
  ...
  const tabs = ['Information', 'Content', 'Campaigns', ...(isOwnProfile ? ['My Activity'] : []), 'Goatnet'];
        ...
        <div className="interests-grid">
          {profile.interests.map(interest => (
            <div key={interest.id} className="interest-item">
              <span className="interest-dot" style={{ backgroundColor: interest.color }}></span>
              {interest.label}
            </div>
          ))}
        </div>
...
const MilestonesSection = ({
  ...
  canEdit: boolean;
  onAddMilestone: (milestone: { title: string; date: string; description?: string }) => void;
}) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAddMilestone({ title: title.trim(), date: date.trim(), description: description.trim() || undefined });
    setTitle('');
    setDate('');
    setDescription('');
    setIsAdding(false);
      ...
      {canEdit && isAdding && (
        <form className="milestones-form" onSubmit={handleSubmit}>
          ...
          <div className="milestones-form-actions">
            <button type="submit" className="btn-primary milestones-save-button">
              Save milestone
            </button>
            <button
              type="button"
          ...
          {sorted.map((milestone, index) => (
            <div key={milestone.id} className="milestone-row">
              <div className="milestone-marker">
                ...
                {index !== sorted.length - 1 && <div className="milestone-line" />}
              </div>
  ...
  return (
    <div className="media-section">
      <div className="media-player">
        <img src={profile.media.thumbnailUrl} alt="Featured Media" className="media-thumbnail" />
...
const SocialMetricsPanel = ({ socials }: { socials: SocialProfile[] }) => {
  if (!socials.length) return null;
  return (
    <section className="panel social-panel">
      <h3 className="section-title">Social reach & engagement</h3>
      <div className="social-metrics-grid">
        {socials.map((s) => (
          <div key={s.platform} className="social-card">
            <div className="social-header">
              <span className="social-platform">{formatPlatformLabel(s.platform)}</span>
            ...
            <div className="social-stats">
              ...
              <div>
                ...
                <span className="social-stat-value">{s.avgEngagementRate}%</span>
              </div>
              <div>
...
const AudiencePanel = ({ audience }: { audience: AudienceBreakdown }) => {
  ...
  const hasDetailedData = audience.ageBands?.length || audience.locations?.length || audience.interests?.length;
  const hasSimpleData = audience.primaryMarkets?.length || audience.targetAudience;
    ...
    return (
      <section className="campaigns-section">
        ...
        <p className="campaigns-empty">
          No paid collaborations logged yet — great opportunity for a first-mover partner.
        </p>
      </section>
    );
  }
      ...
      <div className="campaigns-grid">
        {campaigns.map((campaign) => (
          <article key={campaign.id} className="campaign-card">
            ...
            {campaign.quote && <p className="campaign-quote">“{campaign.quote}”</p>}
          </article>
        ))}
      </div>
    </section>
...
const ContactCTA = ({ channels }: { channels: ContactChannel[] }) => {
  const orgChannels = channels.filter((ch) => ch.type === 'org' || ch.type === 'agent');
    ...
    <section className="contact-cta">
      <div>
        ...
        <p className="contact-copy">
          Share your goal — awareness, camp sign-ups, traffic, or product launches — and the BlueBlood NIL
          desk will help design a program with Anabella that fits your budget and timeline.
        </p>
      ...
      <div className="contact-actions">
        {primary.email && (
          <a className="btn-primary contact-btn" href={`mailto:${primary.email}`}>
            Email NIL desk
          </a>
        )}
        <button className="btn-secondary contact-btn">Download media kit</button>
      </div>
    </section>
  );
};
...
export const AthleteProfile = () => {
  ...
  useEffect(() => {
    async function fetchAthleteProfile() {
      ...
      try {
        ...
        let feedPosts = [];
        try {
          feedPosts = await apiClient.get<any[]>(`/api/athletes/${id}/feed-posts`);
        } catch (feedErr) {
          console.error('Failed to fetch feed posts:', feedErr);
          // Don't fail the whole profile load if feed posts fail
        }
        
        const transformed = transformAthleteProfile({ ...data, todos }, feedPosts);
        setProfile(transformed);
        setMilestones(
          (transformed.milestones || []).slice().sort((a, b) => b.date.localeCompare(a.date))
        );
      } catch (err) {
  ...
  const handleAddMilestone = async (milestone: { title: string; date: string; description?: string }) => {
    try {
      ...
      setMilestones((prev) =>
        [newMilestone, ...prev].slice().sort((a, b) => b.date.localeCompare(a.date))
      );
    } catch (error) {
  ...
  return (
    <div className="profile-container">
      ...
      <div className="profile-content-wrapper">
        ...
        {activeTab === 'Information' && (
          <div className="profile-content-grid">
            ...
            <div className="right-column">
              ...
              <SocialMetricsPanel socials={profile.socialProfiles} />
        ...
        {activeTab === 'Campaigns' && <CampaignsSection campaigns={profile.campaigns} />}
        ...
        {activeTab === 'My Activity' && isOwnProfile && (
          <div className="activity-tab-content">
            ...
            <TodosList 
              todos={profile.todos || []}
              campaigns={profile.campaignsWithEarnings.map(c => ({ id: c.id, title: c.title }))}
              onTodoUpdate={() => {
                // Refetch todos when updated
                if (isOwnProfile) {
                  apiClient.get<any[]>('/api/athletes/me/todos')
                    .then(todos => {
                      setProfile(prev => prev ? { ...prev, todos } : null);
                    })
                    .catch(err => console.error('Failed to refresh todos:', err));
                }
              }}
            />
          </div>
