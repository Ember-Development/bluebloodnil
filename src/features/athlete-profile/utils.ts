...
export function transformAthleteProfile(
  ...
  const socialProfiles = (athlete.socialProfiles || []).map((sp) => ({
    ...
    avgEngagementRate: sp.avgEngagementRate ?? undefined, // Keep undefined instead of converting to 0
    avgViews: sp.avgViews ?? undefined, // Keep undefined instead of converting to 0
  ...
  const campaigns = (athlete.participants || [])
    ...
    .map((p) => ({
      ...
      brandLogoUrl:
        p.campaign.organization.logoUrl || "https://placehold.co/64x64",
      title: p.campaign.title,
      objective: p.campaign.description || "",
      deliverables: [],
      outcomes: [],
      results: (p.campaign as any).results || null, // Add this line to include results
    }));
  ...
  const brandContent: BrandContentItem[] = (athlete.contents || [])
    .map((content) => {
      ...
      const associatedCampaign = campaigns.find(
        (c) =>
      ...
      return {
        id: content.id,
        type: (content.type === "video" ? "video" : "image") as
          | "image"
          | "video"
          | "post",
  ...
  return {
    ...
    brandFitSummary:
      athlete.brandFitSummary || "Brand positioning information coming soon.",
    interests,
    ...
    campaignsWithEarnings: (athlete.participants || [])
      ...
      .map((p) => ({
        ...
        brandLogoUrl:
          p.campaign.organization.logoUrl || "https://placehold.co/64x64",
        status: (p.campaign.status === "ACTIVE"
          ? "active"
          : p.campaign.status === "COMPLETED"
            ? "completed"
            : p.campaign.status === "ARCHIVED"
              ? "cancelled"
              : "pending") as "active" | "completed" | "pending" | "cancelled",
